...
function levelDot(l) {
  const t = l.action
  if (t.includes('报修') || t.includes('接单') || t.includes('隔离') || t.includes('检修') || t.includes('确认恢复')) return 'repair'
  if (t.includes('阻止')) return 'block'
...
.dot.repair{background:#fb8c00;}.dot.block{background:#ab47bc;}
