  ...
  <div class="scenes">
    ...
    <div class="list">
      <div v-for="s in store.scenes" :key="s.id" class="scene" :class="{off:!s.enabled}">
        ...
        <div class="actions">
          <div v-for="a in s.actions" :key="a.id" class="act-chip" :class="{invalid:!a.device_name, iso:isIsolated(a)}">
            <span class="k">{{ a.device_name || a.device_key || '未知设备' }}<em v-if="!a.device_name">{{ a.unresolved === 'duplicate' ? '重名·待重新绑定' : '已删除' }}</em><em v-else-if="isIsolated(a)">🔒隔离中</em></span>
...
const isIsolated = (a) => a.device_id && store.devices.find((d) => d.id === a.device_id)?.isolated
...
.act-chip.iso{border-color:rgba(255,179,0,0.45);}
